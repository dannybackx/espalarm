...
// This is in struct tm in glibc
static long int tm_gmtoff;
static /* const */ char *tm_zone;
...
update_vars (void)
{
  int newtz = -tz_rules[0].offset / 3600;
  // Debug("update_vars {"); Debug(tz_rules[0].name); Debug(","); Debug(tz_rules[1].name);
  // { char buffer[64]; sprintf(buffer, ",tz offset %d", newtz); Debug(buffer); }
  // Debug("}\n");
  ...
  size_t len0 = strlen (tzname[0]);
  size_t len1 = strlen (tzname[1]);
...
{
  //
  int r = min (ss, 59) + min (mm, 59) * 60 + min (hh, 24) * 60 * 60;
  // Debug("compute_offset("); DebugInt(ss); Debug(",");
  // DebugInt(mm); Debug(",");
  // DebugInt(hh); Debug(") -> ");
  // DebugInt(r); Debug("\n");
  //
...
{
  // Debug("parse_tzname("); Debug(tzp[0]); Debug(","); DebugInt(whichrule); Debug(")\n");
  ...
  // Debug("parse_tzname -> "); Debug(tz_rules[whichrule].name); Debug(")\n");
...
parse_offset (const char **tzp, int whichrule)
{
  ...
  *tzp = tz;
  ...
  if (sscanf (tz, "%hu%n:%hu%n:%hu%n",
    ...
    // Debug("parse_offset -> "); DebugInt(tz_rules[whichrule].offset); Debug(", line "); DebugInt(__LINE__); Debug(" consumed "); DebugInt(consumed); Debug("\n");
  } else
...
{
  // Debug("parse_rule("); Debug(tzp[0]); Debug(","); DebugInt(whichrule); Debug(")\n");
  const char *tz = *tzp;
  ...
  if (*tz == 'J' || isdigit (*tz))
	...
	return false;
      tz += consumed;
      // Debug("parse_rule -> "); DebugInt(tzr->m);
      // Debug(","); DebugInt(tzr->n);
      // Debug(","); DebugInt(tzr->d);
      // Debug("\n");
    }
  else if (*tz == '\0')
    {
...
{
  // Debug("__tzset_parse_tz("); Debug(tz); Debug(")\n");
  ...
  if (parse_tzname (&tz, 0) && parse_offset (&tz, 0))
      ...
      if (*tz != '\0')
	{
	  if (parse_tzname (&tz, 1))
	    {
	      parse_offset (&tz, 1);
	      // Debug("YOW "); DebugInt(__LINE__); Debug("tz "); Debug(tz); Debug("\n");
	      if (*tz == '\0' || (tz[0] == ',' && tz[1] == '\0'))
		{
...
// HERE struct tm * __tz_convert (const time_t *timer, int use_localtime, struct tm *tp)
  update_vars ();
}
...
/* Figure out the correct timezone for TM and set `tzname',
   `timezone', and `daylight' accordingly.  */
void
__tz_compute (time_t timer, struct tm *tm, int use_localtime)
{
  ...
  if (use_localtime)
	...
	isdst = (timer >= tz_rules[0].change
      ...
      tm->tm_isdst = isdst;
      // tm->tm_zone = tzname[isdst];
      /* tm-> */ tm_gmtoff = tz_rules[isdst].offset;
      // Debug("__tz_compute : isdst "); DebugInt(isdst); Debug("\n");
    }
}

